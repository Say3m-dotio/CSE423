...
WINDOW_TITLE = b"Highway Havoc: Depth Test Enabled"
...
def draw_circle_manual(radius, color):
    """Draws a flat circle using ONLY GL_TRIANGLES (Pizza Slices)."""
    glPushMatrix()
    glColor3f(*color)
    glBegin(GL_TRIANGLES)
    ...
    for i in range(num_segments):
        ...
        # draw triangle (1 slice of pizza) 
...
def draw_barrier(x, z):
    ...
    # warning stripes {black n yellow}
...
#collisions
        ...
        for b in barriers:        # barrier collision {high damage}
...
def main():
    ...
    print("=== HIGHWAY HAVOC: DEPTH TEST EDITION ===")
    print("Controls: Arrows, Space (Nitro), C (Cheat), L (Lights), R (Restart)")
    print("Features: Magnet, Barriers, Circular Coins (GL_TRIANGLES only)")
    glutMainLoop()
